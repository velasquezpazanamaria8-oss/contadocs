...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action']??'') === 'eliminar_doc') {
    $doc_id = $_POST['doc_id'] ?? '';
    $doc = Database::fetch(
        "SELECT d.* FROM documentos d JOIN empresas_cliente ec ON d.empresa_id=ec.id WHERE d.id=? AND ec.estudio_id=?",
  ...
  <div class="app-main">
    <div class="topbar">
      <div class="topbar-left">
        ...
        <p>Todos los documentos de tus clientes</p>
      </div>
      <div class="topbar-actions">
        <a href="/contador/subir.php" class="btn btn-primary btn-sm">
    ...
    <div class="app-content">
      ...
      <div class="metrics-grid" style="grid-template-columns:repeat(3,1fr);margin-bottom:20px">
        ...
        <div class="metric-card green">
          <div class="metric-top">
            ...
            <div class="metric-icon green"><svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg></div>
          </div>
        </div>
      ...
      <form method="GET" style="display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap;align-items:center">
        <div style="position:relative;flex:1;min-width:160px">
          ...
          <input type="text" name="q" value="<?= e($buscar) ?>" placeholder="Buscar..." class="form-input" style="padding-left:32px">
        </div>
        <select name="empresa_id" class="form-select" style="width:auto;min-width:160px" onchange="this.form.submit()">
          <option value="">Todas las empresas</option>
          <?php foreach ($empresas as $emp): ?>
          <option value="<?= e($emp['id']) ?>" <?= $emp_id_filtro===$emp['id']?'selected':'' ?>><?= e($emp['razon_social']) ?></option>
          <?php endforeach; ?>
        </select>
        <select name="periodo" class="form-select" style="width:auto" onchange="this.form.submit()">
          <option value="">Todos los períodos</option>
          <?php foreach ($periodos as $p): ?>
          <option value="<?= e($p['periodo']) ?>" <?= $periodo_filtro===$p['periodo']?'selected':'' ?>><?= e($p['periodo']) ?></option>
          <?php endforeach; ?>
        </select>
      ...
      <div class="card">
        ...
        <div class="empty-state">
          ...
          <div class="empty-sub"><?= ($emp_id_filtro||$periodo_filtro||$buscar)?'Prueba cambiando los filtros':'Sube el primer documento' ?></div>
        </div>
        <?php else: ?>
              ...
              $render_doc_cells = function($doc, $mostrar_empresa) {
                ...
                <td>
                  <div style="display:flex;flex-direction:column;gap:2px">
                    <span class="badge <?= $doc['total_descargas']>0?'badge-green':'badge-gray' ?> badge-none"><?= $doc['total_descargas'] ?> <?= $doc['total_descargas']==1?'vez':'veces' ?></span>
                    <?php if ($doc['ultima_descarga']): ?>
                    <span style="font-size:10px;color:var(--gris-400)"><?= tiempoRelativo($doc['ultima_descarga']) ?></span>
                    <?php endif; ?>
                  </div>
                </td>
                <td>
...
<div class="modal-overlay" id="modalEliminar" onclick="if(event.target===this)cerrarModal()">
  <div class="modal">
    ...
    <div class="alert alert-error"><svg fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" width="16" height="16"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>El cliente ya no podrá descargarlo.</div>
    <form method="POST">
